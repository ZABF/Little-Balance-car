...
//			speed_integral = 0;
		...
		if(*(uint32_t *)&uart_rx[1] == 'WROF'){//FORW
			...
			if(target_movement != 0){
				velocity_kp = velocity_kp_temp;//-7;
				flag_movement = 1;
			}else{
				velocity_kp = velocity_kp_temp;
				flag_movement = 0;
...
void Data_Send_Status(float Pitch, float Roll, float Yaw,int16_t *Accel,int16_t *Gyro){
    unsigned char i = 0;
    ...
    data_to_send[_cnt++] = BYTE0(Accel[2]);//ACC
    ...
    data_to_send[_cnt++] = BYTE0(Gyro[2]);//GYRO
		data_to_send[_cnt++] = 0x00;
		data_to_send[_cnt++] = 0x00;
		data_to_send[_cnt++] = 0x00;
		data_to_send[_cnt++] = 0x00;
		data_to_send[_cnt++] = 0x00;
		data_to_send[_cnt++] = 0x00;//MAG
...
//		control_motor(&htim2,6720,Fast);
//		control_motor(&htim5,6720,Fast);
    ...
    {	
        ...
        {
            hal.next_temp_ms = timestamp + TEMP_READ_MS;//temperture read every 500ms
            new_temp = 1;
        }
        ...
        {
            short gyro[3], accel_short[3], sensors_short;
            unsigned char more;
            long accel[3], quat[4], temperature;
					
            dmp_read_fifo(gyro, accel_short, quat, &sensor_timestamp, &sensors_short, &more);
            if (!more)
                hal.new_gyro = 0;
            if (sensors_short & INV_WXYZ_QUAT)
            {
                inv_build_quat(quat, 0, sensor_timestamp);
                new_data = 1;
            }
            if (sensors_short & INV_XYZ_GYRO)
            {
            ...
            if (sensors_short & INV_XYZ_ACCEL)
            {
