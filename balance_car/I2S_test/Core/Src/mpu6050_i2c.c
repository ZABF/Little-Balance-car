...
uint8_t MPU_Write_Len(uint8_t add,uint8_t reg, uint8_t len, uint8_t *buf)
{
	...
	uint8_t i;
	I2C_Start();
	I2C_SendByte((add << 1) | 0); //发送器件地址+写命令
	...
	HAL_I2C_Mem_Write(&MPU6050_I2C_Handle, (add << 1), reg, I2C_MEMADD_SIZE_8BIT, buf, len, 0xfff);
	return 0;
#endif
}
/**
...
uint8_t MPU_Read_Len(uint8_t add,uint8_t reg, uint8_t len, uint8_t *buf)
{
	...
	return 0;
#elif defined(MPU6050_Hardware_I2C)
	HAL_I2C_Mem_Read(&MPU6050_I2C_Handle, (add << 1), reg, I2C_MEMADD_SIZE_8BIT, buf, len, 0xfff);
	HAL_Delay(1);
	return 0;
#endif
}
...
uint8_t MPU_Write_Byte(uint8_t reg, uint8_t data)
{
	...
	return 0;
#elif defined(MPU6050_Hardware_I2C)
	return HAL_I2C_Mem_Write(&MPU6050_I2C_Handle, (MPU_ADDR << 1), reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 0xfff);
#endif
}
/**
 * @brief MUP6050 I2C写一个字节
...
#elif defined(MPU6050_Hardware_I2C)
	uint8_t zj;
	HAL_I2C_Mem_Read(&MPU6050_I2C_Handle, (MPU_ADDR << 1), reg, I2C_MEMADD_SIZE_8BIT, &zj, 1, 0xfff);
	return zj;
#endif
}
